...
const categoryIcons: Record<string, ComponentType<IconProps>> = {
  electrico: BatteryIcon,
  gasolinero: EngineIcon,
};
    ...
    <section className="mx-auto max-w-370 px-5 py-7 lg:px-12">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        {categories.map((category) => {
          const CategoryIcon = category.slug
            ...
            >
              ...
              <div className="relative z-10 flex min-h-58 min-w-0">
                <div className="flex min-w-0 flex-1 flex-col justify-between p-5 sm:p-6">
                  <div>
                    {CategoryIcon ? (
                      <span className="mb-5 grid h-12 w-12 place-items-center rounded bg-white/10 text-white ring-1 ring-white/15 transition group-hover:bg-red-600 group-hover:ring-red-500">
                        <CategoryIcon className="h-6 w-6" />
                      </span>
                    ) : null}
                    <h3 className="max-w-60 wrap-break-word font-display text-[1.35rem] font-black uppercase leading-none text-white sm:text-2xl">
                      {category.name}
                    </h3>
