...
export function HomeHero() {
  return (
    <section className="relative overflow-hidden border-b border-neutral-200 bg-[linear-gradient(100deg,#fff_0%,#fff_48%,#f4f4f4_48%,#f7f7f7_100%)]">
      ...
      <div className="mx-auto grid min-h-140 max-w-370 items-center gap-8 px-5 py-12 sm:gap-10 sm:py-16 lg:grid-cols-[0.9fr_1.1fr] lg:px-12">
            ...
            {trustItems.map(({ Icon, title, text }) => (
              <div key={title} className="flex min-w-0 items-center gap-3">
                ...
                <div className="min-w-0">
                  <p className="wrap-break-word text-xs font-black uppercase text-neutral-900">
                    {title}
                  </p>
                  <p className="wrap-break-word text-[0.72rem] font-semibold text-neutral-500">
                    {text}
                  </p>
                </div>
              </div>
        ...
        <div className="relative z-10 grid min-h-80 overflow-hidden place-items-center sm:min-h-115 lg:min-h-130">
          <Image
            src="/karbin/hero-karbin.webp"
            alt="Kit de repuestos Karbin Motors"
            width={1536}
            height={1024}
            preload
            quality={60}
            sizes="(min-width: 1280px) 760px, (min-width: 1024px) 52vw, (min-width: 640px) 620px, 92vw"
            className="relative z-20 w-full max-w-105 -rotate-6 object-contain drop-shadow-2xl sm:max-w-155 lg:max-w-190"
          />
        </div>
      </div>
    </section>
